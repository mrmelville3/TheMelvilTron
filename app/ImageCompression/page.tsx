...
export default function Page() {
    ...
    return (
        <div>
            ...
            <Paragraph>
                Singular Value Decomposition (SVD) is one of the tools used in some artificial intelligence applications. It is a mathematical operation that takes a matrix as input and factors it into three new matrices called U, Sigma (Σ), and V transposed (V<sup>T</sup>). These matrices have unique and valuable properties. The values of the original matrix are distributed and reorganized across the three new matrices such that the initial columns of U and the initial rows of V<sup>T</sup> contain the most characteristic information from the original matrix. Likewise, the latter columns of U and latter rows of V<sup>T</sup> add little to no value when reconstructing the original matrix and can be left out altogether in many cases.
            </Paragraph>
            <Paragraph>
                Image compression is a way to visually illustrate key concepts of SVD. The original image is not a matrix, but it can be separated into matrices for colors red, green, and blue. SVD is applied to each of them producing U, Σ, and V<sup>T</sup> for each color. Using the slider and plus/minus buttons you can set how much of U, Σ, and V<sup>T</sup> are used to reconstruct the image. The image is reconstructed by performing standard matrix multiplication on the parts of U, Σ, and V<sup>T</sup> matrices included by the slider. However, if you include too much of these matrices, the size of the data needed to rebuild the image exceeds the size of the original image, and we have not achieved compression at all. The compressed red, green, and blue images are then combined to recreate the final compressed image in full color.
            </Paragraph>
            <Paragraph>
